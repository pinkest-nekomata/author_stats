select
  ...
  , sales_channel_fee_tax_per_unit as sales_channel_per_unit_raw
  ...
  , round(publisher_share * coalesce(split_share,1),2)     as my_share_lcur
  , round(publisher_share_usd * coalesce(split_share,1),2) as my_share_usd
...
from {{ ref('stg__d2d_ebook_sales') }} as d2d_esales
left join {{ ref('books') }} as books
    on d2d_esales.ebook_isbn::varchar = books.ebook_isbn::varchar
